...
void Init_ADC(void) {
	...
	ADC0->CFG1 = 0x9C; // 16 bit // low power
	ADC0->SC2 = 0;
}
...
uint32_t LCD_TS_Read(PT_T * position) {
	uint32_t x, y;
	uint32_t b;
	static osStatus_t result_TS;
	static MSG_REQ req_msg;
	static MSG_RES res_msg_in_LCD_TS;
...
//		yu = ADC0->R[0];
		...
		req_msg.cnum = LCD_TS_YU_CHANNEL;
		req_msg.result_id = ADC_RES_ID;
		osMessageQueuePut(ADC_REQ_ID,&req_msg,NULL,osWaitForever);
		result_TS = osMessageQueueGet(ADC_RES_ID,&res_msg_in_LCD_TS,NULL,osWaitForever);
		if (result_TS == osOK)
		{
			x = res_msg_in_LCD_TS.adc_conv;
		}
#endif
		//x = yu;
...
//		xl = ADC0->R[0];
		...
		req_msg.cnum = LCD_TS_XL_CHANNEL;
		req_msg.result_id = ADC_RES_ID;
		osMessageQueuePut(ADC_REQ_ID,&req_msg,NULL,osWaitForever);
		result_TS = osMessageQueueGet(ADC_RES_ID,&res_msg_in_LCD_TS,NULL,osWaitForever);
		if (result_TS == osOK)
		{
			y = res_msg_in_LCD_TS.adc_conv;
		}
#endif
//		y = xl;
